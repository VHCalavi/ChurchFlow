...
export default function AdministrationPage() {
  const [activeTab, setActiveTab] = useState<"EQUIPMENT" | "PROVIDERS">("EQUIPMENT");
  ...
  const [status, setStatus] = useState<"EXCELLENT" | "BON" | "MAUVAIS" | "EN_PANNE">("EXCELLENT");
  ...
  const [equipmentList, setEquipmentList] = useState<Equipment[]>([
    { id: "e1", name: "Console de mixage Behringer X32", category: "SONORISATION", quantity: 1, status: "EXCELLENT", value: "1 850 000 F" },
    { id: "e2", name: "Microphone HF Shure SM58", category: "SONORISATION", quantity: 4, status: "BON", value: "650 000 F" },
    { id: "e3", name: "Vidéoprojecteur Epson 4K", category: "MULTIMEDIA", quantity: 2, status: "BON", value: "1 200 000 F" },
    { id: "e4", name: "Chaises rembourrées VIP", category: "MOBILIER", quantity: 150, status: "EXCELLENT", value: "3 750 000 F" },
    { id: "e5", name: "Climatiseur Split LG 2CV", category: "ELECTRICITE", quantity: 6, status: "EN_PANNE", value: "1 500 000 F" },
  ]);

  const [providerList, setProviderList] = useState<Provider[]>([
    { id: "p1", name: "Cotonou Sound & Light", service: "Maintenance Sonorisation", phone: "+229 97 11 22 33", status: "ACTIF" },
    { id: "p2", name: "Clean Space SARL", service: "Entretien & Nettoyage", phone: "+229 95 44 55 66", status: "ACTIF" },
    { id: "p3", name: "Electro Clime Service", service: "Réparation Climatisation", phone: "+229 61 77 88 99", status: "INACTIF" }
  ]);
  ...
  const getStatusColor = (s: string) => {
    switch (s) {
      ...
      case "BON": return "bg-primary/5 text-primary border border-primary/20";
      case "MAUVAIS": return "bg-amber-50 text-amber-700 border border-amber-200";
      case "EN_PANNE": return "bg-red-50 text-red-700 border border-red-200";
    ...
    <DashboardLayout title="Administration Matérielle & Prestataires">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-5 mb-8 rounded-xl border border-slate-100 bg-white shadow-[0px_3px_4px_0px_rgba(0,0,0,0.03)]">
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder={activeTab === "EQUIPMENT" ? "Rechercher un équipement, matériel..." : "Rechercher un prestataire, service..."}
            value={searchTerm}
        ...
        >
          ...
          <span>{activeTab === "EQUIPMENT" ? "Ajouter un équipement" : "Ajouter un prestataire"}</span>
        </button>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                {equipmentList.filter(e => e.name.toLowerCase().includes(searchTerm.toLowerCase())).map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/30 transition-colors">
                    <td className="py-4 px-6 font-semibold text-slate-900 flex items-center space-x-3.5">
                      <Package className="w-5 h-5 text-slate-400" />
                      <span>{item.name}</span>
                    </td>
                    <td className="py-4 px-6 text-slate-500 text-xs font-semibold uppercase">{item.category}</td>
                    <td className="py-4 px-6 text-slate-800 font-semibold">{item.quantity}</td>
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider ${getStatusColor(item.status)}`}>
                        {item.status}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-primary font-semibold">{item.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-xl border border-slate-100 bg-white shadow-[0px_3px_4px_0px_rgba(0,0,0,0.03)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                {providerList.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map((provider) => (
                  <tr key={provider.id} className="hover:bg-slate-50/30 transition-colors">
                    <td className="py-4 px-6 font-semibold text-slate-900 flex items-center space-x-3.5">
                      <Users className="w-5 h-5 text-slate-400" />
                      <span>{provider.name}</span>
                    </td>
                    <td className="py-4 px-6 text-slate-800 font-semibold">{provider.service}</td>
                    <td className="py-4 px-6 text-slate-500 text-xs font-semibold">{provider.phone}</td>
                    <td className="py-4 px-6">
          ...
          <div className="w-full max-w-lg p-6 bg-white rounded-xl border border-slate-100 shadow-premium">
            <div className="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">
                {activeTab === "EQUIPMENT" ? "Ajouter un Équipement" : "Ajouter un Prestataire"}
              </h3>
              <button 
            ...
            {activeTab === "EQUIPMENT" ? (
              <form onSubmit={handleAddEquipment} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Nom de l&apos;Équipement *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Catégorie *</label>
                    <select
                      value={category}
                    ...
                    >
                      ...
                      <option value="ELECTRICITE">Électricité & Froid</option>
                      <option value="AUTRE">Autre</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Quantité *</label>
                    <input
                      type="number"
                      required
                      min={1}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">État Général *</label>
                    <select
                      value={status}
                      onChange={(e) => setStatus(e.target.value as "EXCELLENT" | "BON" | "MAUVAIS" | "EN_PANNE")}
                      className="w-full px-3.5 py-2.5 text-sm font-semibold rounded-lg border border-slate-200 bg-slate-50 text-slate-900 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all cursor-pointer"
                    >
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Valeur estimée *</label>
                    <input
                      type="text"
                      required
              ...
              <form onSubmit={handleAddProvider} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Nom de l&apos;Entreprise prestataire *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Service fourni *</label>
                  <input
                    type="text"
                    required
