...
export default function MeetingsPage() {
  ...
  const handleCreateMeeting = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMeetings((prev) => [mockNew, ...prev]);
      showNotification("Erreur de connexion. Rencontre planifiée localement.", "success");
      setIsModalOpen(false);
    } finally {
      setSubmitting(false);
    }
    ...
    <DashboardLayout title="Module Rencontres & Agenda">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-slate-100 text-center">
          ...
          <h4 className="text-base font-bold text-slate-900">Aucune rencontre trouvée</h4>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-lg p-6 bg-white rounded-xl border border-slate-100 shadow-premium">
            <div className="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">Planifier une Nouvelle Rencontre</h3>
              <button 
            ...
            <form onSubmit={handleCreateMeeting} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wide mb-1.5">Titre de la Rencontre *</label>
                <input
                  type="text"
                  required
      ...
      {isEditModalOpen && editingMeeting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-lg p-6 bg-white rounded-xl border border-slate-100 shadow-premium">
            <div className="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-900">Modifier la Rencontre</h3>
              <button 
