...
export function MemberDetailsDrawer({ memberId, onClose }: Props) {
  const [tab, setTab] = useState<"general" | "groupes" | "presences">(
    "general",
  );
  ...
  const presentCount =
    member?.meetingsAttended?.filter((a) => a.isPresent).length ?? 0;
          ...
          {!loading && member && tab === "general" && (
            <div className="space-y-3.5">
              {[
                ...
                {
                  label: "Genre",
                  value: member.gender === "HOMME" ? "Homme" : "Femme",
                },
                { label: "Email", value: member.email || "Non renseigné" },
                { label: "Téléphone", value: member.phone || "Non renseigné" },
                {
                  label: "Date d'entrée",
                  value: new Date(member.joinDate).toLocaleDateString("fr-FR"),
                },
                {
                  label: "Baptême",
                ...
                <div
                  key={label}
                  className="flex justify-between py-2.5 border-b border-[#E0E5F2]/40 dark:border-white/5"
                >
              ...
              {member.groups.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm font-medium text-[#A3AED0]">
                    Aucun groupe assigné à ce membre.
                  </p>
                </div>
              ) : (
          ...
          {!loading && member && tab === "presences" && (
            <div className="space-y-5">
              ...
              <div className="p-5 rounded-2xl bg-[#E6FAF5]/80 dark:bg-[#0B2A22]/20 border border-[#01B574]/20 flex items-center justify-between">
                <div className="flex flex-col items-start">
                  <p className="text-sm font-medium text-[#01B574]">
                    Taux de présence
                  </p>
                  <p className="text-3xl font-extrabold text-[#01B574] mt-1">
                    {rate}%
                  </p>
              ...
              <div className="space-y-2.5">
                {(member.meetingsAttended?.length ?? 0) === 0 ? (
                  <p className="text-sm font-medium text-[#A3AED0] text-center py-6">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex flex-col items-start">
                          ...
                          <p className="text-xs font-medium text-[#A3AED0] mt-0.5">
                            {a.meeting?.date
                              ? new Date(a.meeting.date).toLocaleDateString(
                                  "fr-FR",
                                )
                              : ""}
                          </p>
