...
import { TrendingUp, TrendingDown, Award, Users, Loader2, AlertCircle } from "lucide-react";
...
export function AttendanceTrendChart() {
    ...
    async function fetchData() {
      try {
        ...
        if (cultes.length === 0) {
          ...
          setData(allWithAttendance.map(m => ({
            label: formatDateLabel(m.date),
            value: m._count.attendees > 0
  ...
  const avgRate = data.length > 0
    ...
    : 0;
  const avgPresent = data.length > 0
    ? Math.round(data.reduce((s, d) => s + d.presentCount, 0) / data.length)
    : 0;
  const peakPoint = data.length > 0
    ? data.reduce((best, d) => d.presentCount > best.presentCount ? d : best, data[0])
    : null;
  ...
  const maxVal = data.length > 0 ? Math.min(100, Math.max(...allValues) + 5) : 100;
  ...
  if (data.length === 1) {
    ...
    for (let i = 0; i < data.length - 1; i++) {
      const x1 = getX(i);       const y1 = getY(data[i].value);
      const x2 = getX(i + 1);   const y2 = getY(data[i + 1].value);
    ...
    <div className="p-6 rounded-xl border border-slate-100 bg-white shadow-[0px_3px_4px_0px_rgba(0,0,0,0.03)] hover:shadow-premium transition-all duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100">
        <div>
          <div className="flex items-center space-x-2">
            <h3 className="text-base font-bold text-slate-900 tracking-tight">Taux de Présence Global</h3>
        ...
        <div className="flex items-center space-x-1 text-xs font-bold" style={{ color: isTrendUp ? "#10b981" : "#ef4444" }}>
          ...
          <span>{trend >= 0 ? "+" : ""}{trend}% sur la période</span>
        </div>
      </div>
      ...
      {!loading && !error && data.length === 0 && (
        <div className="flex flex-col items-center justify-center py-14 text-center">
          ...
          <p className="text-sm font-semibold text-slate-500">Aucune présence enregistrée pour le moment.</p>
          <p className="text-xs text-slate-400 mt-1">Les données apparaîtront après le premier émargement.</p>
        </div>
      )}
        ...
        <>
            ...
            <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible">
              ...
              {yGridLevels.map((level) => (
                <g key={level} className="opacity-40">
                  ...
                  />
                  <text x={paddingLeft - 6} y={getY(level) + 4}
              ...
              {/* Bezier line */}
              {linePath && (
                <path d={linePath} fill="none" stroke="url(#lineGradient)"
              ...
              {hoveredIndex !== null && (
                <line
                  x1={getX(hoveredIndex)} y1={paddingTop}
                  x2={getX(hoveredIndex)} y2={paddingTop + chartHeight}
                  stroke="#006C69" strokeWidth="1" strokeDasharray="3,3" opacity="0.35"
                />
              )}
              ...
              {data.map((point, index) => {
                ...
                return (
                  <g key={index}
                    ...
                    {/* Glow */}
                    {isHovered && (
                      <circle cx={x} cy={y} r="9" fill="#006C69" fillOpacity="0.12" />
                    )}
                    {/* Dot */}
                    <circle cx={x} cy={y}
                      r={isHovered ? "6" : "4"}
                    ...
                    <text x={x} y={height - 6} textAnchor="middle"
                      fontSize="9" fontWeight={isHovered ? "800" : "600"}
          ...
          {hoveredIndex !== null && (
            <div className="mt-3 px-4 py-3 rounded-xl bg-primary/5 border border-primary/10 flex items-center justify-between animate-fade-in">
              <div>
                <p className="text-xs font-bold text-primary uppercase tracking-wide">{data[hoveredIndex].label}</p>
                <p className="text-sm font-semibold text-slate-700 mt-0.5 truncate max-w-[200px]">{data[hoveredIndex].meetingTitle}</p>
              </div>
              <div className="text-right">
                <p className="text-xl font-extrabold text-primary">{data[hoveredIndex].value}%</p>
                <p className="text-xs font-semibold text-slate-500">
                  {data[hoveredIndex].presentCount} présents / {data[hoveredIndex].totalRecorded} enregistrés
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 mt-5 pt-4 border-t border-slate-50">
            ...
            <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100">
              <div className="flex items-center justify-center space-x-1 mb-1.5">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Moyenne</span>
              </div>
              <p className="text-sm font-extrabold text-slate-900">{avgRate}%</p>
              <p className="text-[10px] font-semibold text-slate-400 mt-0.5">≈ {avgPresent} pers./culte</p>
            </div>
            ...
            <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100">
              <div className="flex items-center justify-center space-x-1 mb-1.5">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Record</span>
              </div>
              {peakPoint && (
                <>
                  <p className="text-sm font-extrabold text-slate-900">{peakPoint.value}%</p>
                  <p className="text-[10px] font-semibold text-slate-400 mt-0.5">{peakPoint.presentCount} présents — {peakPoint.label}</p>
                </>
              )}
            </div>
            ...
            <div className="text-center p-3 rounded-xl bg-primary/5 border border-primary/10">
              <div className="flex items-center justify-center mb-1.5">
                <span className="text-[10px] font-bold text-primary/70 uppercase tracking-wide">
                  {hoveredIndex !== null ? data[hoveredIndex].label : "Dernier culte"}
                </span>
              </div>
              {(() => {
                const pt = hoveredIndex !== null ? data[hoveredIndex] : data[data.length - 1];
                return (
                  <>
                    <p className="text-sm font-extrabold text-primary">{pt.value}%</p>
                    <p className="text-[10px] font-semibold text-primary/60 mt-0.5">
