...
export function Topbar({ title = "Tableau de Bord Global" }: TopbarProps) {
  ...
  const [activeNotifTab, setActiveNotifTab] = useState<"ALL" | "INBOX" | "TEAM" | "FOLLOWING">("ALL");
  ...
  const navbarBg = isDark ? "rgba(11, 20, 55, 0.5)" : "rgba(244, 247, 254, 0.2)";
    ...
    <>
      ...
      >
        ...
        <div style={{ marginBottom: "0px" }}>
          ...
          <nav className="flex items-center gap-1.5 mb-[5px]" aria-label="breadcrumb">
          ...
          <h1 className={`font-bold leading-none ${textCls}`} style={{ fontSize: "34px" }}>
            {title}
          </h1>
        </div>
            ...
            {isProfileOpen && (
              <>
                ...
                <div
                  ...
                  style={{ background: menuBg, boxShadow: menuShadow, borderRadius: "20px" }}
                >
                  {/* Greeting header */}
                  <div
                  ...
                  <div className="p-2.5 flex flex-col gap-0.5">
                    <Link
                      ...
                      onMouseEnter={(e) => (e.currentTarget.style.background = isDark ? "rgba(255,255,255,0.06)" : "#F4F7FE")}
                    ...
                    <button
                      ...
                      style={{ borderRadius: "8px" }}
                      onMouseEnter={(e) => (e.currentTarget.style.background = isDark ? "rgba(238,93,80,0.1)" : "#FEEFEE")}
                      onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                    >
      ...
      {isNotifOpen && (
        <div
          ...
          style={{ background: "rgba(11,20,55,0.30)", backdropFilter: "blur(2px)" }}
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: `1px solid ${borderColor}` }}>
          <div className="flex flex-col items-start gap-0.5">
            <h2 className={`text-base font-bold ${textCls}`}>Notifications</h2>
            <button className="text-xs font-semibold" style={{ color: isDark ? "#7551FF" : "#422AFB" }}>
        ...
        {/* Tabs */}
        <div className="flex items-center px-6" style={{ borderBottom: `1px solid ${borderColor}` }}>
          {(["ALL", "INBOX", "TEAM", "FOLLOWING"] as const).map((tab) => (
            ...
            >
              {tab === "ALL" ? "Toutes" : tab === "INBOX" ? "Messages" : tab === "TEAM" ? "Équipe" : "Suivis"}
              {tab === "INBOX" && (
                <span className="ml-1 inline-block w-1.5 h-1.5 rounded-full bg-emerald-500" />
              )}
            </button>
          ))}
          <button className={`ml-auto p-1.5 rounded-lg ${iconCls} hover:opacity-70 transition-all`} style={{ borderRadius: "8px" }}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6" style={{ scrollbarWidth: "none" }}>
          ...
          <div className="flex space-x-4 items-start">
            <div className="relative flex-shrink-0">
              <div className="w-10 h-10 rounded-full text-white flex items-center justify-center font-bold text-xs" style={{ background: "#11047A" }}>
                JL
              </div>
              <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-500" style={{ boxShadow: `0 0 0 2px ${menuBg}` }} />
            </div>
            <div className="flex-1 min-w-0">
              <p className={`text-xs font-semibold ${textCls}`}>
                <span className="font-extrabold">Joe Lincoln</span> vous a mentionné dans{" "}
                <span className="font-bold" style={{ color: isDark ? "#7551FF" : "#422AFB" }}>Membres Actifs</span>
              </p>
              <p className={`text-[10px] font-semibold mt-1 ${subTextCls}`}>Il y a 18 min • Administration</p>
              <div
                className={`mt-3 p-4 text-xs font-medium ${textCls}`}
                style={{ background: isDark ? "rgba(11,20,55,0.8)" : "#F4F7FE", borderRadius: "12px" }}
              >
                @Admin Est-ce que les fiches d&apos;inscription des nouveaux bergers ont été validées ?
              </div>
            </div>
          </div>

          {/* Notif 2 */}
          <div className="flex space-x-4 items-start pt-6" style={{ borderTop: `1px solid ${borderColor}` }}>
            <div className="relative flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-xs font-semibold ${textCls}`}>
                <span className="font-extrabold">Leslie Alexander</span> a mis à jour{" "}
                <span className="font-bold" style={{ color: isDark ? "#7551FF" : "#422AFB" }}>Permissions Équipe</span>
              </p>
              <p className={`text-[10px] font-semibold mt-1 ${subTextCls}`}>Il y a 53 min • Sécurité</p>
              <div className="flex flex-wrap gap-2 mt-3">
                <span
                  className="text-[9px] font-bold px-2 py-0.5 rounded"
                  style={{ background: isDark ? "rgba(117,81,255,0.15)" : "#F2EFFF", color: isDark ? "#7551FF" : "#422AFB" }}
                >
                  Responsable
                </span>
                <span
                  className="text-[9px] font-bold px-2 py-0.5 rounded"
                  style={{ background: isDark ? "rgba(1,181,116,0.1)" : "#E6FAF5", color: "#01B574" }}
                >
                  GEM
                </span>
              </div>
        ...
        <div
          ...
          style={{ borderTop: `1px solid ${borderColor}`, background: isDark ? "rgba(11,20,55,0.6)" : "#F4F7FE" }}
        >
          <button
            onClick={() => setIsNotifOpen(false)}
      ...
      {isSearchOpen && (
        <div
          ...
          style={{ background: "rgba(11,20,55,0.40)", backdropFilter: "blur(2px)" }}
          onClick={() => setIsSearchOpen(false)}
        >
          <div
            className="w-full max-w-xl flex flex-col overflow-hidden"
            style={{ background: menuBg, boxShadow: menuShadow, borderRadius: "20px", maxHeight: "70vh" }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Input row */}
            <div className="flex items-center px-5 py-4 gap-3" style={{ borderBottom: `1px solid ${borderColor}` }}>
            ...
            {/* Sub-tabs */}
            <div className="flex items-center px-5 overflow-x-auto" style={{ borderBottom: `1px solid ${borderColor}`, scrollbarWidth: "none" }}>
              {[
                ...
                <button
                  ...
                  className={`py-3.5 px-3 text-[11px] font-bold border-b-2 whitespace-nowrap transition-all ${
            ...
            <div
              ...
              style={{ background: isDark ? "rgba(11,20,55,0.3)" : "rgba(244,247,254,0.3)", scrollbarWidth: "none" }}
            >
              <div className="space-y-3">
                <h4 className={`text-[10px] font-bold uppercase tracking-widest ${iconCls}`}>
                  Actions rapides
                </h4>
                <div className="flex flex-col gap-2">
                  {[
                    { href: "/dashboard/profile", icon: <User className="w-4 h-4" />, label: "Mon profil", bg: "#F2EFFF", color: "#422AFB" },
                    { href: "/dashboard/administration", icon: <Settings className="w-4 h-4" />, label: "Configuration de l'application", bg: "#E6FAF5", color: "#01B574" },
