...
# Ou en mode production / sans migration interactive :
# pnpm exec prisma db push
```

> Cette commande lit `prisma/schema.prisma` et synchronise la structure avec la base de données.
...
```bash
# Toujours depuis packages/database
pnpm db:generate
...
pnpm --filter @churchflow/database db:seed
```
